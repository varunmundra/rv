class Kyc < ActiveRecord::Base
	...
	validate :correspondence_landline_or_mobile, :if => :active_or_c_address? 
	...
	validate :permanent_landline_or_mobile, :if => :active_or_p_address? 
	...
	def correspondence_landline_or_mobile
	  if [self.c_landline, self.c_mobile].reject(&:blank?).size == 0
	    errors[:base] << ("Please fill either landline no. or mobile no.")
	  end
	end 
	def permanent_landline_or_mobile
	  if [self.p_landline, self.p_mobile].reject(&:blank?).size == 0
	    errors[:base] << ("Please fill either landline no. or mobile no.")
	  end
	end		
